...
#               return pythoncopy.deepcopy(self)
        ...
        def select_font(self, fontpath=None, fontsize=20):
                if fontpath is None:
                        fontpath = "/home/orhan/gif_animated_image/A-Track-master/images/f2n/arial.ttf"
                else:
                        fontpath = fontpath
                return imft.truetype(fontpath, fontsize)
        ...
        def drawcircle(self, x, y, r=10, colour=None, label=None, fontsize=40):
                ...
                if label is not None:
                        # The we write it :
                        self.loadlabelfont()
                        # textwidth = self.draw.textsize(label, font=self.select_font(fontsize=fontsize))[0]
                        bbox = self.draw.textbbox((0, 0), label, font=self.select_font(fontsize=fontsize))
                        textwidth = bbox[2] - bbox[0]
                        self.draw.text((pilx - float(textwidth)/2.0 + 2,
                                        pily + pilr + 4),
                                       label,
                                       fill=colour,
                                       font=self.select_font(fontsize=fontsize))
        ...
        def writeinfo(self, text, xpos, ypos, colour=None, fontsize=20):
                ...
                self.draw.text((xpos, ypos), text,
                                       fill=colour, font=self.select_font(fontsize=fontsize))
