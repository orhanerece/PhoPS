...
from f2n import f2n
from gaia_builder import DynamicAstrometry

gain = 0.57
rdnoise = 4.11
pxscale = 0.62
ast = DynamicAstrometry("bf_2059_0016_V.fits")
# ast.get_header_coords()
# ast.download_gaia_patch()
# ast.generate_healpix_tiles()
# ast.build_index_files()
# gain = 0.9
# rdnoise = 5.50
# pxscale = 0.32641
...
def zeropoint(sources, x, y, magerr, plot=False, show=False, path=None):
    ...
    print("SLOPE", zmag)
    if plot:
        def rotated_bounding_box(x, y, margin=0.0):
            """
            x, y : 1D arrays (inlier points)
            returns: 4x2 array of box corners
            """
            ...
            xmin, ymin = rot.min(axis=0) - margin
            xmax, ymax = rot.max(axis=0) + margin
        ...
        # Inlier noktalar (geometry için 1D!)
        x_in = r_resultant[inlier_mask].ravel()
        y_in = mag_diffs[inlier_mask].ravel()

        # Rotated box
        rect, angle = rotated_bounding_box(x_in, y_in, margin=0.00)
        plott = True
        if plott:
            from matplotlib.patches import Polygon

            plt.scatter(r_resultant, mag_diffs, s=5, color="black")
            plt.scatter(x_in, y_in, s=5, color="blue", label="Inliers")
            # plt.plot(xaxis, yaxis, "o", markersize=2, color="#1f77b4",
            #          label="Gaia DR3 sources: {}".format(len(xaxis)))
            plt.plot(r, zmag(r), markersize=2, label="linear fit-slope {}".format(str("%.4f" % slope)),
                     linestyle='dashed',
                     color="#ff7f0e")
            ...
            plt.ylabel("Δmag")
            plt.legend()
            save = os.path.splitext(path)[0] + "x.pdf"
        ...
        plt.plot(xaxis, yaxis, "o", markersize=2, color="#1f77b4",
                 label="Gaia DR3 sources: {}".format(len(xaxis)))
        plt.plot(r, zmag(r), markersize=2, label="linear fit-slope {}".format(str("%.4f" % slope)), linestyle='dashed',
                 color="#ff7f0e")
        plt.xlabel("r - distance to the center (pixel)")
        plt.ylabel("Gaia DR3 sources mag differences (std - inst)")
...
# files = sorted(glob.glob("/home/orhan/Desktop/Tug-H18-I/observations/20221114/1620/bf_*.fits", recursive=True))[7:8]
# files = sorted(glob.glob("/home/orhan/Desktop/temp/orhnerc/vundtia_Empty_110.fits", recursive=True))
# files = sorted(glob.glob("/home/orhan/phops_article/bf_*.fits", recursive=True))[0:1]
# files = sorted(glob.glob("/home/orhan/phops_article/202514115e.fts", recursive=True))[0:1]
files = sorted(glob.glob("/home/orhan/Phops_article_version/bf_2059_0016_V.fits", recursive=True))[0:1]
...
for path in files:
    ...
    # object_plot(path, int(x_center), int(y_center), id)  # cropped image
    ...
    print(asteroid)
    allinfo = np.asarray(asteroid, dtype=object)
    print(allinfo)
    ...
    ascii.write(centers, "phot_{}GaiaHaumea.dat".format(str(id).replace(" ", "")),
